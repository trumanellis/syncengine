...
//!
//! Minimal terminal aesthetic from DESIGN_SYSTEM_V2.md.
...
:root {
  ...
  /* Type Scale (simplified: 3 sizes) */
  --text-sm: 0.875rem;   /* 14px */
  --text-base: 1rem;     /* 16px */
  --text-lg: 1.5rem;     /* 24px */

  /* Legacy type scale aliases (for backward compatibility) */
  --text-xs: 0.75rem;
  --text-xl: 1.5rem;
  --text-2xl: 2rem;
  --text-3xl: 3rem;
  ...
  /* Legacy transition aliases (for backward compatibility) */
...
.network-page {
  min-height: 100vh;
  background: var(--void-black);
  padding: 0; /* Removed padding for chat layout - chat fills full screen */
}
...
.network-chat-layout {
  ...
  /* Account for: NavHeader (~60px) + bottom nav (~80px) */
...
@media (max-width: 768px) {
  ...
  .conversation-panel {
    /* Ensure panel fills available height */
    height: 100%;
  }
...
/* ========================================
   PACKET LAYER STYLES - Design System v2
   ======================================== */
...
.conversation-empty .empty-text {
  font-size: var(--text-base);
  color: var(--text-secondary);
}

.conversation-empty .empty-hint {
  font-size: var(--text-sm);
  color: var(--text-muted);
}
